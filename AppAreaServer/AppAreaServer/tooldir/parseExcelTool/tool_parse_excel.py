...
#命名空间
SERVER_NAME_SPACE = "ExcelConf"
SERVER_NAME_SPACE_TBX = "ExcelTbx"
...
#创建配置管理文件头文件
def create_cpp_head():
    cppCode = "#ifndef __EXCEL_H__\n"
    cppCode += "#define __EXCEL_H__\n"
    ...
    cppCode += "#include \"excelBase.h\"\n"
    cppCode += "\nnamespace %s \n{\n" %(SERVER_NAME_SPACE_TBX)
    return cppCode

#创建配置管理文件实现文件
def create_cpp_headcpp():
    cppCode = "#include \"excel.h\"\n\n"
    cppCode += "namespace %s \n{\n" %(SERVER_NAME_SPACE_TBX)
    return cppCode
...
def create_cpp_cpp_function(classArr):
    ...
    cppCode = "" 
    for key in classArr:
        cppCode += "\t%s::ExcelTable<%s::%s,%s::Conf_t_%s,%s::%s::t_%s> _%s;\n" %(SERVER_NAME_SPACE_TBX,SERVER_NAME_SPACE,key,SERVER_NAME_SPACE,key,SERVER_NAME_SPACE,key,key,key)
    
    cppCode += "\n"
    cppCode += "\tvoid ExcelManager::init(const std::string &fileName,const %s::ReadExcelFunc &func)\n" %(SERVER_NAME_SPACE_TBX)
    cppCode += "\t{\n"
    cppCode += "\t\tloadConf(fileName);\n"
    for key in classArr:
        cppCode += "\t\tloadTable(\"%s.%s\",_%s,func);\n" %(SERVER_NAME_SPACE,key,key) 
    cppCode += "\t}\n"
    
    cppCode += "\n"
    for key in classArr:
        cppCode += "\tconst %s::ExcelTable<%s::%s,%s::Conf_t_%s,%s::%s::t_%s >& %s()\n" %(SERVER_NAME_SPACE_TBX,SERVER_NAME_SPACE,key,SERVER_NAME_SPACE,key,SERVER_NAME_SPACE,key,key,key)
        cppCode += "\t{\n"
        cppCode += "\t\t return _%s;\n\t}\n" %(key)
...
def create_code():
    ...
    os.system("/usr/local/protobuf/bin/protoc -I={0} --python_out={1} --cpp_out={2} {3}".format(SERVER_XML_DIR_NAME,SERVER_PYTHON_DIR_NAME,SERVER_CPP_DIR_NAME,protoName))
...
#生成book中的所有sheet的tbx
def create_book_tbx_file(fileName,keyMap):
    result = []
    if not check_analysis_sheet_name(fileName,result) or len(result) != 3:
        print "错误: %s 文件名解析错误" %(fileName)
        return 
    if int(result[1]) == SHEET_TYPE_IGNORE or int(result[1]) == SHEET_TYPE_ONLY_CHECK:
        print "警告: %s 被忽略" %(fileName)
        return
    if not check_english_name(result[2]):
    ...
    file = result[2]
    posIndex = file.find(".xlsx")
    if posIndex == -1 or posIndex == len(file)-1:
        print "错误: %s 文件名非法" %(fileName)
        return
    temp = file[:posIndex] 
    pyFileName = temp + "_pb2.py"
    ...
    mod = import_mod(temp +"_pb2",SERVER_PYTHON_DIR_NAME)
    table = data.sheets()
    for sheet in table:
...
def write_tbx_xml_node():
    ...
    for file in fileArr:
        ...
        xmlCode += "\t<file name=\"%s.%s\"  path=\"%s\"/>\n" %(SERVER_NAME_SPACE,extendName,file)
    xmlCode += "</tbx>\n"
    return xmlCode
...
#生成一个表格所有的txt文件
def write_txt_data(data,keyArr):
    for sheet in data.sheets():
        write_txt_sheet(sheet,keyArr)
        

#解析excel表
def parseExcel(fileName,classArr):
    print "提示: %s 开始被解析" %(fileName)
    result = []
    if not check_analysis_sheet_name(fileName,result) or len(result) != 3:
        print "错误: %s 文件名解析错误" %(fileName)
        return False
    if int(result[1]) == SHEET_TYPE_IGNORE:
        print "警告: %s 被忽略" %(fileName)
        return True
    if not check_english_name(result[2]):
        print "错误: %s 文件名对应英文非法" %(fileName)
    ...
    if not check_sheet(data,keyMap,classArr):
        return False
    posIndex = result[2].find(".")
    if posIndex == -1:
        return False
   
    #生成xml文件
    name = result[2][:posIndex]
    xmlName = CLIENT_XML_DIR_NAME + "/" + name + ".xml" 
    xml_fp = file(xmlName,"wb")
    xml_fp.write(write_xml_declaration())
    xml_fp.write(write_xml_root(data,name,keyMap))
    xml_fp.close()
     
    #生成txt文件
    write_txt_data(data,keyMap)
    ...
    #生成tbx二进制文件
    create_book_tbx_file(fileName,keyMap)
...
def create_cpp_head_file(classArr):
    cppName = MANAGER_CPP_DIR_NAME + "/" + "excel" + ".h"
...
def create_cpp_cpp_file(classArr):
    cppName = MANAGER_CPP_DIR_NAME + "/" + "excel" + ".cpp"
    cpp_fp = file(cppName,"wb")
...
def main():
    ...
    create_dir(CLIENT_XML_DIR_NAME)
    ...
    create_dir(CLIENT_TXT_DIR_NAME)
